...
if (existsSync(scheduleFile)) {
  ...
  for (const line of content.split("\n")) {
    ...
    else if (line.includes("โ๏ธ")) {
      const dest = line.match(/โ’([A-Z]+)/)?.[1];
...
for (const line of cal.split("\n")) {
  ...
  for (const day of dayNums) {
    ...
    if (day === todayNum) {
      ...
      marked = marked.replace(new RegExp(`\\s${day}(\\s|$)`), `ยฐ${day}$1`);
    } else if (info && info.type !== "done") {
      marked = marked.replace(new RegExp(`\\s${day}(\\s|$)`), `ยท${day}$1`);
    }

    if (info) {
      switch (info.type) {
        case "flight": annotations.push(`${day}โ๏ธ${info.event || ""}`); break;
